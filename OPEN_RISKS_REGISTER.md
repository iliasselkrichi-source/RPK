...
| R-013 | Manual address fallback may create a booking with zero calculated distance if Google services are unavailable | Operational risk | Fallback allows booking submission when Places/Directions fail, preserving the lead instead of blocking the customer | Operations should manually confirm price for bookings where route data could not be calculated; future phase can add server-side pricing fallback | OPEN |
